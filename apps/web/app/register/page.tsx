  ...
  async function handleGoogleAuth() {
    ...
    try {
      ...
      if (res.authUrl) {
        ...
        setError("Google sign-up couldn't be started. Please try again.");
      }
    } catch (err: any) {
      setGoogleLoading(false);
      setError(
        err instanceof ApiError && err.status !== 500
          ? err.message
          : "Google sign-up couldn't be started. Please try again.",
      );
    }
  }
  ...
  return (
    <main className="relative mx-auto flex min-h-screen max-w-sm sm:max-w-md flex-col justify-center gap-5 p-6">
      ...
      <Button
        ...
        className="w-full flex items-center justify-center gap-2.5 py-2.5 sm:py-3 text-xs sm:text-sm font-semibold transition-all hover:opacity-95 border border-glass-border/40"
        aria-label={googleLoading ? 'Connecting to Google…' : 'Register with Google'}
      >
        ...
        {googleLoading && (
          <svg className="h-3.5 w-3.5 animate-spin text-info shrink-0" viewBox="0 0 24 24" fill="none">
